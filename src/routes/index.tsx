...
function MonitorScreen() {
  ...
  const confirmSaveCapture = useCallback(async () => {
    ...
    const formatError = getNameFormatError(trimmedName)
    if (formatError) {
    ...
    try {
      ...
      if (response.status === 409) {
        ...
        setNameError(data?.error || 'This name is already used by another snapshot.')
        return
      }

      if (!response.ok) {
        const data = await response.json().catch(() => null)
        setNameError(data?.error || 'Capture could not be saved. Try again.')
        return
      }
  ...
  const confirmRename = useCallback(async () => {
    ...
    const formatError = getNameFormatError(trimmed)
    if (formatError) {
    ...
    try {
      ...
      if (response.status === 409) {
        const data = await response.json().catch(() => null)
        setRenameError(data?.error || 'This name is already used by another snapshot.')
        return
      }

      if (!response.ok) {
        const data = await response.json().catch(() => null)
        setRenameError(data?.error || 'Could not rename this capture. Try again.')
        return
      }
                  ...
                  {captures.map((capture) => (
                    <div className="capture-thumb" key={capture.id}>
                      ...
                      {predictions[capture.id] === 'error' && (
                        <div className="thumb-prediction thumb-prediction-error">
                          <ShieldAlert size={13} />
                          <span>Analysis failed</span>
                        </div>
                      )}
                      {predictions[capture.id] &&
                        predictions[capture.id] !== 'loading' &&
                        predictions[capture.id] !== 'error' && (
                          <div className="thumb-prediction">
                            <Activity size={13} />
                            <span>
                              {(predictions[capture.id] as { predictedHb: number; status: string })
                                .predictedHb}{' '}
                              g/dL —{' '}
                              {(predictions[capture.id] as { predictedHb: number; status: string })
                                .status}
